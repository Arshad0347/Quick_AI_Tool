...
import { Hash, Home, SquarePen, Image, Eraser, Scissors, User, FileText, LogOut } from "lucide-react";
                ...
                <div className="px-6 mt-5 text-sm text-gray-600 font-medium">
                    {navItems.map(({to, label, Icon}) => (
                        <NavLink 
                            ...
                            className={({isActive}) => `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234E4] text-white' : ''}`}
                        >
...
<img src={user.imageUrl} onClick={openUserProfile} className="w-8 rounded-full" alt="" />
<div>
...
Plan
                ...
                <LogOut className="w-4.5 text-gray-400 hover:text-gray-700 transition cursor-pointer" onClick={signOut}></LogOut>
            </div>
        </div>
    );
};
